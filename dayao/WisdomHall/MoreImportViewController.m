...
-(void)addBtn{
    ...
    NSArray * ary = @[@"课堂",@"会议",@"统计",@"校园生活",@"社团",@"校办",@"失物",@"校圈"];
    
    NSArray * aryImageName = @[@"课堂圆图",@"会议圆图",@"统计",@"生活圆图",@"社团圆图",@"校办圆图",@"失物圆图",@"校圈圆图"];
    ...
    for (int i = 0; i<ary.count; i++) {
        ...
        if (i+1==ary.count) {
            UIView * line = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(important.frame)+10+100*(i+1)/2, APPLICATION_WIDTH, 1)];
            line.backgroundColor = [UIColor colorWithHexString:@"#f1f1f1"];
            [_bottomScrollView addSubview:line];
        }
