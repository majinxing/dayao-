...
@property (strong, nonatomic)  UIButton *registerB;
...
-(void)addContentView{
    ...
    _registerB = [UIButton buttonWithType:UIButtonTypeCustom];
    _registerB.frame = CGRectMake(0, CGRectGetMaxY(_password.frame)+13+1+20, 70, 20);
    _registerB.titleLabel.font = [UIFont systemFontOfSize:15];
    [_registerB setTitle:@"注册" forState:UIControlStateNormal];
    [_registerB addTarget:self action:@selector(registerBtn:) forControlEvents:UIControlEventTouchUpInside];
    [_btnBackView addSubview:_registerB];
        ...
        if ([str isEqualToString:@"0000"]) {
            ...
            if ([ss isEqualToString:@"true"]) {
                ...
                if ([type isEqualToString:@"2"]) {
                    [UIUtils showInfoMessage:@"您登陆的是学生身份,本客户端只服务于老师，请登录“律动课堂”" withVC:self];
                }else{
